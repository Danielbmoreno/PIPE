...
export default function StudentSpace() {
  ...
  return <div className="page-shell student-page pipe-space pipe-with-minu">
    <section className="student-hero pipe-space-hero"><div className="hero-copy"><span className="eyebrow">UN MOMENTO PARA TI</span><h1>Mi espacio PIPE</h1><p>{isAdmin ? 'Acompañar a otros también requiere momentos para ti. Encuentra pequeñas pausas, lecturas y motivación para tu día.' : 'Tu vida universitaria también necesita curiosidad, pequeñas pausas y compañía. Este espacio es tuyo.'}</p><div className="hero-actions"><Link className="primary-button" to="/app/mi-espacio?seccion=pausa">Tomarme una pausa</Link><Link className="ghost-button" to="/app/dashboard">{isAdmin ? 'Mi dashboard' : 'Mi proceso'} <span aria-hidden="true">↗</span></Link></div></div><div className="pipe-space-mark" aria-hidden="true">✦</div></section>
    ...
    <ReadingSpace />
    <section className="pipe-space-footer"><span aria-hidden="true">💙</span><p>{isAdmin ? 'Tu bienestar también cuenta. Una pausa puede ayudarte a continuar con tu labor de acompañamiento.' : 'No tienes que recorrer todo el camino hoy. Tu equipo de acompañamiento también está para ti.'}</p><Link to={isAdmin ? '/app/citas' : '/app/mis-citas'} className="text-link">{isAdmin ? 'Revisar citas →' : 'Revisar mis citas →'}</Link></section>
